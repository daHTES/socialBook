    ...
    if (empty($upFirst) or empty($upLast) or empty($upEmailMobile) or empty($upGen)) {
        ...
        if (!preg_match("^[_a-z0-9-]+(\.[_a-z0-9]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,3})$^", $email_mobile)){
            ...
            if(!preg_match("^[0-9]{11}^", $email_mobile)) {
                ...
                $error = 'Email or Mobile number is not corrected. Plz try again.';
            } else {
                ...
                if ($mob > 11 || $mob < 11) {
                    ...
                    $error = 'Password is too short or too long, its not correct';

                } else {

                    if (DB::query('SELECT mobile FROM users WHERE mobile=:mobile', array(':mobile' => $email_mobile))) {

                        $error = 'Mobile number is already in user!';
...
if (isset($_POST['in-email-mobile']) && !empty($_POST['in-email-mobile'])) {
    ...
    if (!preg_match("^[_a-z0-9-]+(\.[_a-z0-9]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,3})$^", $email_mobile)) {
        ...
        if (!preg_match("^[0-9]{11}^", $email_mobile)) {
            $error = 'Email or Phone number is not correct. Plz try again';
        } else {
...
<body>
    <div class="header">
        ...
        <form action="sign.php" method="post">
            ...
            <div class="password-input">
                ...
                <div class="forgotten-account">Forgotten account</div>
            </div>
    ...
    <div class="main">
        <div class="left-side">
            <img src="assets/image/facebook-sign.png" alt="">
        </div>
        <div class="right-side">
            ...
            <div style="color:#212121; font-size:20px">It's free!</div>
